...
Internationalization is a mechanism to create multilingual software that can be adapted to different languages and regions. We make the code language neutral so that we do not have to create a new application when adding support for more languages in the future. 
...
The following figure shows the e-commerce site for Amazon in French and German languages:

![i18n example](/assets/img/posts/aws-sqs-spring-cloud/SQS-Queue.png)
...
It is typically conducted by translators on the user-facing components of the software. Localization goes beyond translating languages, though. It also refers to localizing time and date differences, currency, accounting standards, culturally appropriate images, symbols, and hand gestures, spelling, and other locale-specific components (including the RTL language.
...
Localization is also abbreviated as l10n because there is a total of 10 characters between the first letter 'l' and the last letter 'n'. Localization is the mechanism to create such an application that can be adapted to a specific language and region by adding locale-specific text and components.
...
We will create a web application in this project which will render an HTML page in different languages depending on the user's language selection.
...
![resource-files](/assets/img/posts/spring-i18n/resource-files.png)
