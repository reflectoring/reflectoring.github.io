...
Simply put, cookies are nothing but a piece of information, which is stored on the client-side(browser) and servers use them when communicating with the client.

They are commonly used to track the activity of a website, to customize user sessions, and for servers to recognize users between requests. Another scenario is to store the JWT token in a cookie so the server can recognize that the user is authenticated or not in every request.
...
`setDomain(String)` is another important method of Cookie API. We use it when we want to specify a domain for our cookie:

```java
servletCookie.setDomain("example.com");
```

By doing this we are telling the client to which domain it should send the cookie. Setting the domain not only will send the cookie to the "example.com" domain but also its subdomains "foo.example.com" and "bar.example.com". What happens if we don't set the domain explicitly? In that scenario it will be **set only to the domain that created the cookie, but not to its subdomains.** 
...
### Reading a Cookie
After setting the cookie to response, the server will need to read the cookies sent by the client in every request. 

Method `getCookies()` returns an array of cookies that are sent with the request(`HttpServletRequest`). We can identify our cookie by the cookie name.
...
### Creating a Cookie
In this section we will create a cookie with the same properties that we did using Servlet API. We will use the class `ResponseCookie` for the cookie and class `ResponseEntity` for setting the cookie in the response. They both are defined inside `org.springframework.http` package. `ResponseCookie` has a static method `from(final String name, final String value)` which returns a builder(`ResponseCookieBuilder`) initialized with name and value of the cookie. We can add all the properties that we need and use the method `build()` of the builder to create the `ResponseCookie` as below:
...
```java
 ResponseCookie springCookie = ResponseCookie.from("cookie-name", "spring-cookie")
                    ...
                    .build();
```
After creating it we add the cookie in the header of the response like:
...
### Deleting a Cookie
In the same way, to delete a cookie we will need to create the cookie with the same name and maxAge to 0 and set it to the response header:
...
Cookies provide a way to exchange information between a server and client(browser) to manage sessions, track the website's activity, or remember user preferences.
