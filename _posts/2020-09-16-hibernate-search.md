...
If you have the Spring boot actuator dependency in your project make sure to this property, too:
```yaml
spring:
  elasticsearch:
    rest:
...
For instance, let's look at the `body` field and assume it has the value 'Hibernate is fun'. 
...
You will see two more new terms in the above table `Normalize` and `Analyze`. 

When we analyze a text it means we are 
applying an analyzer which first tokenizes the text and then applies one or more filters such as a lowercase filter (which converts all the text to lowercase) or a stop word filter (which removes common English stop words such as 'is',
'an', 'the' etc). 

Normalizers are similar to analyzers with the difference that normalizers don't apply a tokenizer. 

**On a given field we can 
either apply an analyzer or a normalizer.**

Coming back to `@Field` annotation. We need to apply the `@Field` annotation on all the fields that we wish to search, 
sort need for projection.
...
3. There is nothing special about the `@Field` annotation which is applied to `hashTags` field. I just want to highlight the fact that tokenizer splits the text on any nonalphanumeric character. So a value like '#food#health' will be 
automatically tokenized to `['food','health]`.

4. The `tag` field which is an enum will mostly consist of a single word. We don't need to analyze such fields. So, instead, we can either set the `analyze` property of `@Field` to 
...
5. When we add `@SortableField` to a field, Elasticsearch will optimize the index for sorting operations over those fields.
...
6. We use `@IndexedEmbedded` when we want to perform a search over nested objects fields. For instance, let's say we want to search
...
1. `Index.NO` indicates that the property will be stored in the index document, but it won't be indexed. We won't be able
...
4. `@ContainedIn` makes a `@OneToMany` relationship bidirectional. When the values of this
...
1. **Hibernate search query DSL**: a nice way to write Lucene queries. If you are familiar with Specification and 
...
`Post` entity and those fields still need to be retrieved. If you only want to fetch what's stored in your index anyway and think this database call is redundant, I know just the way. Keep reading on to find out.
...
Let's zoom in on a couple of interesting things here:
