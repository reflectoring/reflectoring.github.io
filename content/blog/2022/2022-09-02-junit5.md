...
Hamcrest is one of the most popular libraries for writing assertions, it offers a fluent API for that enable us to easily write and read object matchers.
...
```
If we are running on a Linux we will see that the test is ignored with the following message:
```
