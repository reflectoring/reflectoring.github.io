...
[AWS Regions and AZs](https://aws.amazon.com/about-aws/global-infrastructure/regions_az) are essential entities of this global infrastructure.**An AWS Region is composed of multiple AZ-availability zones.** An AZ-Availability Zone is a logical data center within a region. **Each AZ is mapped to physical data centers located in that region, with redundant power, networking, and connectivity.**  
...
**A machine image is similar to a pre-built template containing the operating system with some pre-configured applications installed over it.** For example, we can use a machine image for Windows 2016 server with SQL Server or an RHEL Linux with docker, for creating our EC2 instance. We also select an instance family to assign the number of CPUs and RAM for our VM. These range from nano instances, with one virtual CPU, to instance families of high-end configurations.
...
Both ECS and EKS come with a Fargate option for provisioning EC2 instances. AWS Fargate is serverless compute for containers and removes the need to provision and manage servers

## Serverless Compute With Lambda and SAM
With [AWS lambda](https://aws.amazon.com/lambda) we can eliminate activities of provisioning servers of right capacity.
