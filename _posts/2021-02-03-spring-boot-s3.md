...
It is technically possible to store those [binary files in Postgres](https://www.postgresql.org/docs/current/datatype-binary.html) but that is an area where **object stores like S3 might be better suited**.

### Object Store vs File Store 

So you might ask yourself, how is an object store different from a file store? Without going into the gory details,  an object store is a repository, which stores objects in a flat structure, similar to a key-value store. Opposed to file-based storage where we have a hierarchy of files inside folders, inside folders,... **the only thing we need in an object store is the key of the object we want to retrieve**. Additionally, we can provide metadata (data about data) that we attach to the object to further enrich it.
...
## Basic S3 concepts
S3 was one of the first services offered by AWS in 2006. Since then a lot of features have been added but the core concepts of S3 remained: **Buckets** & **Objects**.

### Buckets
Buckets are the container of your objects you want to store. An important thing to note here is that S3 requires the name of the bucket to be globally unique.
...
### Key Delimiter
By default, the "/" character is treated especially if used in an object key. As written above an object store does not use directories or folders but just keys. However, if you use a "/" in your object key, the AWS S3 console will render the object as if it was in a folder. 
...
Spring Cloud gives us the `org.springframework.cloud:spring-cloud-starter-aws` dependency which bundles all the dependencies we need to communicate with S3.
...
### Uploading a File
Now that our bucket is created we are all set to upload a file of our choice. The client provides us with the overloaded `putObject` method. Besides the fine-grained `PutObejctRequest` we can use the function in three ways:
...
Thanks to Java's `streams` we can simply append the transformation to the request.
...
_https://my-awesome-bucket.s3.eu-central-1.amazonaws.com/some-key_
...
Objects can be made public by altering their ACL like the following:
...
You might don't want to make the file private again, because once it was downloaded there is no need to keep it.
...
S3 offers a concept called `pre-signed URLs`. **A pre-signed URL is the link to our object containing an access token, that allows for temporary download** (or upload). We can easily create such a pre-signed URL by specifying the bucket, the object, and the expiration date:
...
In the above snippet, we do so by simply multiply the duration (in seconds) by 1000 (to convert it to milliseconds) and add that to the current time (in UNIX milliseconds).
...
AWS gives us multiple ways to automatically delete objects from a bucket, however we do use S3's concept of `Object Life Cycle rules`. An object life cycle rule basically contains the information *when* to do *what* with the object:
...
- a status of being `ENABLED`, so as soon as this rule is created id is effective
...
A noteworthy around lifecycle rules is that they are far more versatile, as you can use the filter to only apply the rule to objects with a certain key prefix or carry out other actions like archiving of objects.
