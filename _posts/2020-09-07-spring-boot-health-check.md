...
Health Check also plays an important role in the API ecosystem comprising API developers, partners, and third-party developers. The health status of APIs is regularly updated and published in a dashboard. Giving below health check dashboards from twitter APIs:
...
A proactive approach involves monitoring a set of metrics indicating system health. These are more useful since they give us early indications of any deterioriating health of the system giving us time to take mitigating measures.
...
{"status":"UP"}
```
The status - UP indicates the application is running. This default healthcheck aggregates the health status. The status will show DOWN if the application becomes unhealthy anytime during the run. 
...
Micrometer is a library for collecting metrics from JVM-based applications and services and is included in Spring Boot.

We will now integrate spring boot actuator with a monitoring system called Prometheus. Prometheus operates on a pull model by scraping metrics from an endpoint exposed by the application instances at fixed intervals.
...
We will expose the prometheus endpoint by updating the property in our application.properties.
...
Kubernetes is the most commonly used container orchestration system. One of the key features is self-healing which it does by regularly checking the health of the application. If the container goes down, it will be replaced with a healthy instance. This is detected using two properties:
...
Health Indicators can be grouped for specific purposes. For example we can have a group for database health and another for health of our caches.


## Conclusion

We saw how to effectively run Spring Boot Applications by building healh check mechanisms. 
