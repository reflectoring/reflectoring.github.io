---
title: "Resolving Maven Dependencies with a Bill of Materials (BOM)"
categories: [java]
...
In the `pom.xml` file we add them as direct dependencies, purposefully selecting two different version numbers:
...
- **"omitted for duplicate"** means that the same dependency with the *same* version is declared at another place in the dependency tree higher in resolution order or depth and Maven prefers that one.
- **"omitted for conflict"** means that the same dependency with a *different* version is declared at another place in the dependency tree higher in resolution order or depth and Maven prefers that one.
