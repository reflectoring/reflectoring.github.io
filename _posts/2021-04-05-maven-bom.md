...
By executing the command `mvn dependency:tree -Dverbose=true`, we can list all the dependencies including the transitive ones.

We use the verbose mode of this command to see what are the decisions made by Maven concerning transitive dependency resolution.
...
It is a surprise: We started from two dependencies, and in this output, we find out that Maven pulled other dependencies with different versions. The other dependencies are simply transitive. Just imagine the dependency tree that we will have in a multi-module project with hundreds of dependencies.

We can see that there are many versions of the same dependency or the same version of a dependency in different levels of the tree. For example, there are two versions of the `spring-beans` dependency:`5.2.13.RELEASE` and `5.3.5`.

The question that we can have now is how does Maven handle the case where multiple versions of the same dependency are pulled and what does *omitted for duplicate* and *omitted for conflict* mean?
...
So, to help us understand the dependency tree result, Maven indicates for each transitive dependency why it was omitted:
...
Without the verbose mode, the command `mvn dependency:tree` will show the effective dependency tree without the dependencies that were omitted:
...
Here we have an idea about the exact versions of our dependencies that are used in our application. No duplicate dependencies can be found in the dependency tree.
