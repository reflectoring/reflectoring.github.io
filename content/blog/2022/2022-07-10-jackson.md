...
## What is JSON
It stands for JavaScript Object Notation, it's a text-based format for representing structured data based on JavaScript object syntax.
...
On the other hand, the `writeValue` method is used to turn (serialize) POJOs into JSON.
...
### Maven Dependencies
Firstly, we need to add Jackson Maven dependency `jackson-databind` which in turn transitively adds `jackson-annotations` and `jackson-core`
