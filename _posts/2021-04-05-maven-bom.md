...
As developers, we may have many applications using Maven for their dependency management. These applications need upgrades from time to time to be up to date and to add new features or security updates. This easy task, updating dependencies' versions, could turn out to a nightmare because of conflicts that we can have with certain dependencies. The resolution of these dependencies' conflicts can take a lot of our time at the expense of our business code development. To avoid this kind of situation, we can use the Bill Of Material (BOM), a feature that offers easier and safer dependency management.

In this article, we are going to look at dependency management in Maven and we will also explain the BOM with some examples.
...
Let's imagine writing some business code that requires logging the output, using some String utilities, or securing the application. This logic can be implemented in our project, or we can use a library instead. It is better in this case, and it makes sense, to use existing libraries to minimize the amount of code needed to implement.

The use of libraries encourages the reuse since we will rely on other libraries that solve problems similar to ours: These libraries are called dependencies.
...
- **transitive dependencies**: a project that has a POM file with dependencies can be a dependency for other consumer projects. **The current project dependencies are then considered transitive dependencies for other projects**. When maven pulls a direct dependency, it also pulls its transitive dependencies. In other words, if project A depends on B, and B depends on a C and D, then Maven will pull also B, C, and D when trying to resolve A. In other words, **transitive dependencies are dependencies required by direct dependencies**.

## Managing Transitive Dependencies with Maven

Now that we have an overview of the different dependency types in Maven, let's see in detail how Maven deals with transitive dependencies in a large-scale project.
...
For someone who is not aware of transitive dependencies, he will think that using this dependency declaration, only two jars will be pulled. Fortunately, Maven provides a command that will show us what was pulled exactly concerning these two dependencies.

Using the command `mvn dependency:tree`, we can list all the dependencies including the transitive ones. The result is:
...
Second, **Maven takes the approach of nearest transitive dependency in the tree depth and the first in resolution**. To understand this, let's take an example:
...
Maven indicates for each transitive dependency why it was omitted:
...
Adding the desired transitive dependency version as a direct dependency in the POM file will result in being the nearest in depth. This way Maven will select this version. In our previous example, if we wanted version `v1.3` to be selected, then adding the dependency `DT(v1.3)` in the POM file will ensure its selection.

- #### Introduce `dependencyManagement` section in the POM file


For projects with sub-modules, to ensure compatibility and coherence between all the modules, we need a way to provide the same version of a dependency across all sub-modules. Here comes in action the `dependencyManagement` section: it provides a lookup table for Maven to help determine the selected version of a transitive dependency and to centralize dependency information.

A `dependencyManagement`  section contains dependency  elements. Each dependency is a lookup reference for Maven to determine the version to select for transitive dependencies. The version of the dependency is mandatory in this section. However, outside `dependencyManagement`  section, you can omit the version from your dependencies, and Maven will select the correct version of the transitive dependencies from the list of dependencies provided in `dependencyManagement`.
...
This file can be used in our projects in two different ways: as a parent POM or as a dependency. 

Next, we will explain in detail how can this be done.
...
If a version is added to the dependency, this will override what the managed dependencies section defined and the Maven nearest depth version will be applied.
...
With a new version of Spring Boot, a new BOM file will be provided that handles version upgrades and make sure that all the given dependencies work well together. Developers will only care about upgrading the Spring Boot version, the underlying dependencies compatibility was tested by the Spring Boot team.
