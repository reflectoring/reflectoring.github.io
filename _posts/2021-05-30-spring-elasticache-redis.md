...
the data in a key-value pair and supports data structure ranging from `String` and `List` to `hyperloglogs` and `stream`.

In AWS, one of the ways of using Redis for caching is by using the ElastiCache service. ElastiCache is like a container that host's
...
node runs an instance of the Redis server. A group of interrelated nodes is called a **Shard**. In a single Shard, we can have one primary and
...
To use Spring Cloud AWS, first, we need to add Spring cloud AWS BOM(Bill of material). BOM will help us manage
...
Spring Cloud AWS does all the heavy lifting of configuring the caches for us. All we need to do is to provide the name of
...
For instance, in our example application, we have used two caches. One named `product-cache` in `ProductService`:
...
And another named `user-cache` in `UserService`:
...
Technically, Spring Cloud AWS looks for nodes with the same name but since these are **Single Node** clusters the name of
the node is the same as the cluster name.

We also need to define cluster names in the `application.yml`. Spring Cloud AWS will use this to scan the ElastiCache to
find clusters:
...
As we know that for Caching to work in a Spring application we need a `CacheManager` bean. The job of Spring Cloud
...
* Also `ElastiCacheFactoryBean` passes this host and port to `RedisCacheFactory` which then uses Redis clients such as 
  Lettuce to create the connection object which then actually establishes with the nodes and perform required operations.
