...
spring:
  liquibase:
    ...
    databaseChangeLogTable: # To Specify a different table to use for tracking change history. Default is DATABASECHANGELOG
    databaseChangeLogLockTable: # To Specify a different table to use for tracking concurrent Liquibase usage. Default is DATABASECHANGELOGLOCK
    dropFirst: # Indicates whether to drop database schema before running the migration. Do not use this in prod. Default is false
    user: # Login user that needs to be used
    ...
    labels: # Label expression to be used while running liquibase
    parameters: # Parameters map that needs to be passed to liquibase
    rollbackFile: # File to which rollback SQL is written when an update is performed.
    testRollbackOnUpdate: # Whether rollback should be tested before the update is performed. Default is false
```
