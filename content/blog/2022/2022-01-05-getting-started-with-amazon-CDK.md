...
We work with templates and stacks when using AWS CloudFormation. **We create templates in YAML or JSON format to describe our AWS resources with their properties.** A sample template for hosting a web application looks like this:
...
2. **Install CDK Toolkit**: The AWS CDK Toolkit is the primary tool for interacting with the AWS CDK app through the CLI command `cdk`. It is an open-source project in GitHub. Among its capabilities are producing and deploying the AWS CloudFormation templates generated by the AWS CDK. 
...
Let us create a new CDK project using the CDK CLI using the `cdk init` command:
```shell
 mkdir cdk-app 
 cd cdk-app
...
```
We can see two Java classes with names ending with `App` and `Stack` are generated along with a test class in a Maven project. The class with its name ending in `App`  contains the `main()` method and is the entry point of the application. 

We will understand the function of the `App` and the `Stack` classes and build on this further to define our infrastructure resources in the following sections.


## Introducing Constructs - the Basic Building Block
...
Next, we will modify our stack class to define an infrastructure resource: AWS EC2 with a security group in a default VPC:
...
3. The App construct represents the CDK application.
