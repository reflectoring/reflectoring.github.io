...
```
In the code snippet, we are creating a Kinesis Data Stream with `on-demand` capacity mode. The capacity mode of `On-Demand` is used for unpredictable workloads which scale the capacity of the data stream automatically in response to varying data traffic.
...
### Data Transformation in Firehose Delivery Stream
...
### Data Delivery Format of Firehose Delivery Stream
...
We have used the DataStream API of Flink in this code example for processing the streams of access logs contained in the text file used as a source.
Here we are first creating the execution environment using the `StreamExecutionEnvironment` class. Next, we are creating the source for the streaming data. We have used a [text file](https://github.com/thombergs/code-examples/tree/master/aws/kinesis/src/main/resources/apache_access_log) containing the log records from an Apache HTTP server as a source here. Some sample log records from the text file are shown here:
...
- **input**: In the input configuration, we map the streaming source to an in-application data stream. Data flows from in one or more data sources into the in-application data stream. We have configured a Kinesis Data Stream as a data source.
...
When we run our Kinesis Data Analytics application, the Kinesis Data Analytics service creates an Apache Flink job. 
...
The Flink application we built earlier was built independently and then packaged and deployed in Kinesin Data Analytics by uploading the compiled artifact (jar file) to an S3 bucket. 

We can alternately create Flink applications with an interactive development experience using Studio notebooks in the AWS Console as shown below:

{{% image alt="Create Kinesis Data Analytics Notebooks" src="images/posts/aws-kinesis/kda_notebook.png" %}}

A notebook is a web-based development environment that gives an interactive development experience combined with the capabilities of Apache Flink. Studio notebooks use notebooks powered by [Apache Zeppelin](https://zeppelin.apache.org) and use Apache Flink as the stream processing engine.

They allow us to interactively query data streams in real-time and build and run stream processing applications using SQL, Python, and Scala languages. 

We can also model queries with a notebook, using the Apache Flink Table API and SQL in SQL, Python, Scala, or DataStream API in Scala. After that, we promote the Studio notebook to a continuously-running, non-interactive, Kinesis Data Analytics stream-processing application.
...
Kinesis Video Streams is a fully managed service that we can use to ingest streaming video, audio, and other time-encoded data from various capturing devices using an infrastructure provisioned dynamically in the AWS Cloud. 

The ingested data is stored durably with a default retention period of `1` day with a maximum of `10` years. We can build applications to consume the ingested data frame-by-frame, in real-time for low-latency processing.
