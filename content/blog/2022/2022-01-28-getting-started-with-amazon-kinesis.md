...
* e-commerce product search and purchases
* in-game player activity
* feeds from social media networks
* real-time market data from from financial exchanges
...
Examples of consumer applications are a custom application running on EC2, EMR, or Lambda or an Kinesis Data Firehose delivery stream which can use another AWS service such as DynamoDB, Redshift, or S3 to store the results of their processing.

As part of the processing the consumers (such as a custom application running on EC2 or an Kinesis Data Firehose delivery stream) can store their results using an AWS service such as DynamoDB, Redshift, or S3.
...
We can create a Kinesis data stream either by using the AWS Kinesis Data Streams Management Console, from the [AWS Command Line Interface (CLI)](https://aws.amazon.com/cli/) or using the `CreateStream` operation of AWS SDK. 
...
4. **Kinesis Data Analytics**:Kinesis Data Analytics is another fully managed service from the Kinesis family for processing and analyzing streaming data with helpful programming constructs like windowing, sorting, filtering, etc. We can set up the Kinesis Data Stream as a source of streaming data to a Kinesis Data Analytics application which we will explain in the section on [Kinesis Data Analytics](##kinesis-data-analytics).
